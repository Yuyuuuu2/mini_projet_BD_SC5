...
CREATE TABLE Carte(
   c_id INT,
   c_pos_série SMALLINT,
   c_type VARCHAR(20),
   c_nom VARCHAR(100),
   c_langue VARCHAR(30),
   c_rarete VARCHAR(30),
   c_PV SMALLINT,
   c_résistance VARCHAR(50),
   ...
   ext_id INT NOT NULL,
   PRIMARY KEY(c_id),
   UNIQUE(c_id_provient_de),
   FOREIGN KEY(c_id_provient_de) REFERENCES Carte(c_id),
   FOREIGN KEY(paquet_code_SKU) REFERENCES Paquet(paquet_code_SKU),
   FOREIGN KEY(ext_id) REFERENCES Extension(ext_id)
);
...
CREATE TABLE Possède(
   ...
   FOREIGN KEY(c_id) REFERENCES Carte(c_id),
   FOREIGN KEY(atk_id) REFERENCES Attaque(atk_id)
);

CREATE TABLE A_dessiné(
   c_id INT,
   ill_ID INT,
   PRIMARY KEY(c_id, ill_ID),
   FOREIGN KEY(c_id) REFERENCES Carte(c_id),
   FOREIGN KEY(ill_ID) REFERENCES Illustrateur(ill_ID)
);
...
CREATE TABLE Distribue(
   ...
   FOREIGN KEY(paquet_code_SKU) REFERENCES Paquet(paquet_code_SKU),
   FOREIGN KEY(livraison_numero_suivi) REFERENCES Livraison(livraison_numero_suivi),
